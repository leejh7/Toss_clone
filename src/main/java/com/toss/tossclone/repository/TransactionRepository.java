...
public interface TransactionRepository extends JpaRepository<Transaction, Long> {
    ...
    // join 사용
    @Query(value = "select t from Transaction t inner join fetch t.senderAccount sa where sa.accountCode =:accountCode order by t.transferTime desc ")
    List<Transaction> findTransactionBySenderAccountCodeOrderByTransferTimeDesc(@Param("accountCode") String accountCode);


    // join 사용
    @Query(value = "select t from Transaction t inner join fetch t.receiverAccount ra where ra.accountCode =:accountCode order by t.transferTime desc ")
    List<Transaction> findTransactionByReceiverAccountCodeOrderByTransferTimeDesc(@Param("accountCode") String accountCode);


    // join 사용
    @Query(value = "select t from Transaction t inner join fetch t.senderAccount sa inner join fetch t.receiverAccount ra where sa.accountCode =:accountCode or ra.accountCode =: accountCode order by t.transferTime desc ")
